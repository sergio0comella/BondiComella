...
public class PrenotazioneController {
    ...
    private Prenotazione createPrenotazioneFromRS(ResultSet rs) throws SQLException {
        ...
        int annullata = rs.getInt("annullata");
    ...
    public Map<Prenotazione, Utente> getPrenotazioniUtente(int id) throws Exception {
        UtenteController utController = new UtenteController();
        Map<Prenotazione, Utente> prenotazioni = new HashMap<>();

        String query = "SELECT * FROM prenotazione WHERE fk_id_utente=?";
        PreparedStatement ps = this.conn.prepareStatement(query);
        ps.setInt(1,id);
        ResultSet rs = ps.executeQuery();
    ...
    public void pagaPrenotazione(String id) throws SQLException {
        String query1 = "UPDATE prenotazione SET pagata = '1' WHERE id = ? ";
        PreparedStatement ps1 = this.conn.prepareStatement(query1);
        ps1.setString(1, id);
        ps1.execute();
    }

    public void confermaPrenotazione(String id) throws SQLException {
        String query = "UPDATE prenotazione SET annullata = '2' WHERE id = ? ";
        String query2 = "UPDATE postazione SET stato = 'O' WHERE id = (SELECT fk_id_postazione FROM prenotazione WHERE id = ?)";
        PreparedStatement ps = this.conn.prepareStatement(query);
